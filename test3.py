...
def Minimax(curr_state:State,depth,player,MaximizingPlayer):
    if depth==0 or curr_state.game_over:
        return curr_state,evaluate(curr_state.blocks,player)
    if MaximizingPlayer:
        MaxValue = -10000
        best_move=None
...
def evaluate(blocks,player):
    score = get_score(blocks,player)
    return score
...
def eval_box(box,player):
    ...
    for col in box_T: #Score for each row
        score += count_score(col,player)
    #A score for each diagonal
    diags = np.array([box[0][0],box[1][1],box[2][2]])
    diags_2 = np.array([box[2][0],box[1][1],box[0][2]])
